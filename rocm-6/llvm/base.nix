...
stdenv.mkDerivation (finalAttrs: {
  ...
  nativeBuildInputs = [
    ...
    ninja
    git
    #llvmPackagesNoBintools.clang-tools
    (python3Packages.python.withPackages (p: [ p.setuptools ] ++ (lib.optionals (buildDocs || buildMan) [ p.pygments p.pyyaml p.recommonmark p.myst-parser ])))
