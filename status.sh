...
GITHUB_REPO="${GITHUB_REPO:-lunnova/ml.nix}"
COMMIT_SHA="${COMMIT_SHA:-$(git rev-parse HEAD)}"
CHECK_SUITE_ID=""
USE_CHECKS="${GITHUB_RUN_ID:+true}"  # Use checks if GITHUB_RUN_ID is set
...
get_drvs() {
    ...
    #nix-instantiate --strict --eval -E "builtins.mapAttrs (_: x: x.drvPath) (import ./${NIX_FILE} {})"
        #| jq -r '.[]'
}
...
main() {
    ...
    #nix-env -f "./${NIX_FILE}" -qaP --json --drv-path --out-path --show-trace | jq -r 'with_entries(.value = .value.drvPath)'

    echo "Instantiating drvs"
    nix-instantiate --strict --json -E "import ./${NIX_FILE} {}" >&2
    #nix-instantiate --strict --eval -E "builtins.mapAttrs (_: x: x.drvPath) (import ./${NIX_FILE} {})" >&2
